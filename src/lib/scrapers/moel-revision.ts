...
const LIST_URL = "https://www.moel.go.kr/info/lawinfo/revision/list.do";
const BASE_URL = "https://www.moel.go.kr/info/lawinfo/revision/";
...
export async function fetchMoelRevision(): Promise<Notice[]> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`MOEL revision failed: ${res.status}`);
